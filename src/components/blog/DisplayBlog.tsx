...
const DisplayBlog: React.FC<IProps> = ({ blog }) => {
    ...
    const commentPage = blog._id && userLocation['comments'][blog._id];
    const page = new URLSearchParams(useLocation().search).get('page');

    const { data: cts, isFetching, error, isError} = useComments({id: blog._id, limit: 4 , page: commentPage ? commentPage : +(page || '1')});
    ...
    const handleComment = useCallback((body: string) => {
        ...
        addCommentMute({ token: auth.access_token, dispatch, comment: data, page: commentPage ? commentPage : +(page || '1'), limit:4, id: blog._id });
    }, [addCommentMute])
    ...
    }, [blog.likes.length, auth.user?._id]);
    ...
    }, [blog.saved.length, auth.user?._id])

    useEffect(() => {
        if(blog._id){
            if(commentPage){
                navigate(`/blog/${blog._id}?page=${commentPage}`, {replace: true});
            }else{
                if(page) {
                    dispatch(setBlogCommentsLocation({
                        blogId: blog._id,
    ...
    useEffect(() => {
        if(blog._id && page === null && commentPage){
            dispatch(setBlogCommentsLocation({
                blogId: blog._id,
                page: 1
            }))
        }
        else if(blog._id && commentPage && page && parseInt(page) !== commentPage){
            dispatch(setBlogCommentsLocation({
                blogId: blog._id,
                page: +page
            }))
        }
    }, [page, blog._id, commentPage])
                    ...
                    <>
...

                        {
                            auth.user ? <Input callback={handleComment}/>
                                : (
                                    <h5>
                                        请先<Link to={`/login?blog/${blog._id}`}>登录</Link>再评论
                                    </h5>
                                )
                        }
