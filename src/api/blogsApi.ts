...
//无限加载主页blog数据
export const fetchHomeBlogs = ({ pageParam = 1 }) => getAPI(`home/blogs?page=${pageParam}`).then(res => res.data);

export const fetchBlog = async (id?: string) => getAPI(`blog/${id}`).then(res => res.data);
...
export const createBlog = async ({ blog, token, dispatch, navigate }: likeSaveCURDBlogParams) => {
    ...
    navigate && navigate(`/blog/${res.data._id}`);
}

export const updateBlog = async ({ blog, dispatch, token, navigate }: likeSaveCURDBlogParams) => {
    const result = await checkTokenExpire(token, dispatch);
    const access_token = result ? result : token;
    let url = '';
    let profileBlogThumbnail = ''
    ...
    navigate && navigate(`/blog/${res.data.blog._id}`, { replace: true });
}
